...
const Verification: FC<Props> = ({ setRoute, setOpen }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div
        className="bg-gradient-to-t from-[#bea7a7] h-[360px] to-[#688cc3] bg-opacity-60 dark:bg-gradient-to-t
     dark:from-[#333030] dark:to-[#272b31] rounded-lg shadow-xl p-6 w-[90%] max-w-md relative"
            ...
            <div className="flex space-x-4">
              {verifyNumber.map((value, index) => (
                <input
                  key={index}
                  ref={(el) => {
                    inputRefs.current[index] = el; // Assign ref for each input dynamically without returning anything
                  }}
                  type="text"
                  maxLength={1}
                  required
